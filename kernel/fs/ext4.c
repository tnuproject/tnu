...
{
    if (!image || !out || size < 1024 + sizeof(struct ext4_superblock)) {
        return -1;
    }
    const struct ext4_superblock *sb =
        (const struct ext4_superblock *)((const uint8_t *)image + 1024);
    if (sb->magic != 0xef53) {
        return -1;
    }

    bool extents = (sb->feature_incompat & 0x40u) != 0;
    bool huge_file = (sb->feature_ro_compat & 0x8u) != 0;
    bool is_ext4 = extents || huge_file || sb->blocks_count_hi != 0 || sb->desc_size > 32;
    if (!is_ext4) {
        return -1;
    }
    ...
    out->total_bytes = block_size * blocks;
    out->root_inode = 2;
