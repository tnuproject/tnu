...
static const struct util_help help_topics[] = {
    ...
    { "nano", "nano FILE", "Edit a text file with the native TNU nano applet." },
...
#define NANO_MAX_TEXT (64 * 1024)
#define NANO_LINE_MAX 512

static char nano_text[NANO_MAX_TEXT];
...
static int parse_positive_int_arg(const char **p, int *out)
{
    int value = 0;
...
static int nano_read_line(char *buf, size_t cap)
{
    size_t len = 0;
    if (cap == 0) {
        return -1;
...
static size_t nano_load_file(const char *path)
{
    int fd = open(path, O_RDONLY, 0);
    size_t len = 0;
    if (fd < 0) {
    ...
    while (len + 1 < NANO_MAX_TEXT) {
        ssize_t n = read(fd, nano_text + len, NANO_MAX_TEXT - len - 1);
        if (n <= 0) {
            break;
        }
        len += (size_t)n;
...
static int nano_save_file(const char *path, size_t len)
{
    ...
    while (off < len) {
        ssize_t n = write(fd, nano_text + off, len - off);
        if (n <= 0) {
            close(fd);
            return -1;
        }
...
static int nano_line_count(size_t len)
{
    int count = len ? 1 : 0;
...
static int nano_line_bounds(size_t len, int line, size_t *start, size_t *end)
{
    int current = 1;
    size_t pos = 0;
    if (line <= 0 || (len == 0 && line != 1)) {
        return -1;
    }
    ...
    if (current != line || pos > len) {
        return -1;
    }
    *start = pos;
    while (pos < len && nano_text[pos] != '\n') {
        pos++;
    }
    if (pos < len) {
        pos++;
...
static void nano_print_range(size_t len, int first, int count)
{
    int total = nano_line_count(len);
    if (total == 0) {
        println("[empty]");
        return;
    }
    ...
    for (int line = first; line <= total && line < first + count; line++) {
        size_t start;
        size_t end;
        if (nano_line_bounds(len, line, &start, &end) < 0) {
            break;
        }
        print_int(line);
        print("  ");
        write(1, nano_text + start, end - start);
        if (end == start || nano_text[end - 1] != '\n') {
            print("\n");
        }
    }
}

static int nano_insert_at(size_t *len, size_t pos, const char *text)
{
    size_t text_len = strlen(text);
    size_t add = text_len + 1;
    if (pos > *len || *len + add + 1 >= NANO_MAX_TEXT) {
        return -1;
    }
    memmove(nano_text + pos + add, nano_text + pos, *len - pos + 1);
    memcpy(nano_text + pos, text, text_len);
    nano_text[pos + text_len] = '\n';
    *len += add;
    nano_text[*len] = 0;
    return 0;
}

static int nano_insert_line(size_t *len, int line, const char *text)
{
    size_t start;
    size_t end;
    int total = nano_line_count(*len);
    if (line == total + 1 || (*len == 0 && line == 1)) {
        return nano_insert_at(len, *len, text);
    }
    if (nano_line_bounds(*len, line, &start, &end) < 0) {
        return -1;
    }
    (void)end;
    return nano_insert_at(len, start, text);
}

static int nano_delete_line(size_t *len, int line)
{
    size_t start;
    size_t end;
    if (nano_line_bounds(*len, line, &start, &end) < 0) {
    ...
    memmove(nano_text + start, nano_text + end, *len - end + 1);
    *len -= end - start;
    return 0;
}

static int nano_replace_line(size_t *len, int line, const char *text)
{
    size_t start;
    size_t end;
    if (nano_line_bounds(*len, line, &start, &end) < 0) {
...
{
    println("TNU nano commands:");
    println("  text                 append text as a new line");
    println("  .i N text            insert text before line N");
    println("  .r N text            replace line N");
    println("  .d N                 delete line N");
    println("  .p [N] [COUNT]       print file lines");
    println("  .g N                 show lines around N");
    println("  .f text              find text");
    println("  .w [FILE]            save");
    println("  .q                   quit when clean");
    println("  .q!                  quit without saving");
    println("  .h                   help");
}

static int nano_find_text(size_t len, const char *needle)
{
    char *hit = strstr(nano_text, needle);
    int line = 1;
    if (!hit || !needle[0]) {
        return -1;
    }
    for (char *p = nano_text; p < hit && (size_t)(p - nano_text) < len; p++) {
        if (*p == '\n') {
            line++;
        }
    }
...
static int cmd_nano(int argc, char **argv)
{
    ...
    len = nano_load_file(path);

    print("\033[2J\033[H");
    print("TNU nano: ");
    println(path);
    nano_print_range(len, 1, 20);
    println("^O save  ^X quit: use .w and .q  (.h for help)");

    for (;;) {
        print("nano> ");
        if (nano_read_line(line, sizeof(line)) < 0) {
            return 1;
        }
        ...
        if (line[0] == '.') {
            ...
            if (line[1] == 'h' && line[2] == 0) {
                nano_show_help();
            } else if (line[1] == 'q' && line[2] == '!') {
                return 0;
            } else if (line[1] == 'q' && line[2] == 0) {
                if (dirty) {
                    println("nano: modified; use .w to save or .q! to discard");
                } else {
                    return 0;
                }
            } else if (line[1] == 'w') {
                ...
                if (nano_save_file(path, len) < 0) {
                    println("nano: write failed");
                } else {
                    dirty = 0;
                    print("nano: wrote ");
                    println(path);
                ...
                int first = 1;
                int count = 24;
                if (parse_positive_int_arg(&p, &first) == 0) {
                    parse_positive_int_arg(&p, &count);
                }
                nano_print_range(len, first, count);
            } else if (line[1] == 'g') {
                int target;
                if (parse_positive_int_arg(&p, &target) < 0) {
                    println("usage: .g N");
                } else {
                    int first = target > 5 ? target - 5 : 1;
                    nano_print_range(len, first, 11);
                }
            } else if (line[1] == 'f') {
                p = skip_spaces(p);
                int found = nano_find_text(len, p);
                if (found < 0) {
                    println("nano: not found");
                } else {
                    print("nano: found at line ");
                    print_int(found);
                    print("\n");
                    nano_print_range(len, found > 2 ? found - 2 : 1, 5);
                }
            } else if (line[1] == 'd') {
                int target;
                if (parse_positive_int_arg(&p, &target) < 0 ||
                    nano_delete_line(&len, target) < 0) {
                    println("usage: .d N");
                } else {
                    dirty = 1;
                }
            } else if (line[1] == 'i' || line[1] == 'r') {
                int target;
                const char *text;
                if (parse_positive_int_arg(&p, &target) < 0) {
                    println(line[1] == 'i' ? "usage: .i N text" : "usage: .r N text");
                    continue;
                }
                text = skip_spaces(p);
                if (line[1] == 'i') {
                    if (nano_insert_line(&len, target, text) < 0) {
                        println("nano: insert failed");
                    } else {
                        dirty = 1;
                    }
                } else {
                    if (nano_replace_line(&len, target, text) < 0) {
                        println("nano: replace failed");
                    } else {
                        dirty = 1;
                    }
                }
            } else {
                println("nano: unknown command; use .h");
            }
        } else {
            if (nano_insert_at(&len, len, line) < 0) {
                println("nano: buffer full");
            } else {
                dirty = 1;
            }
        }
...
int main(int argc, char **argv)
{
    ...
    if (strcmp(cmd, "nano") == 0) return cmd_nano(argc, argv);
